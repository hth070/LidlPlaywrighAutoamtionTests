...
*/
        ...
        try {
            ...
            await expect(page.locator('.s-page-heading__text')).toHaveText('8166 produktov');
        } catch (error) {
    ...
    try {
        ...
        const značkaZobraziťVšetkoButton = await page.locator('button[data-testselector="brand-facet-show-more-button"]');
        await expect(značkaZobraziťVšetkoButton.first()).toHaveText('Zobraziť všetko');
    ...
    try {
        ...
        await expect(elementŽltá.first()).toHaveText('žltá');
          ...
          try {
            // Verify the "veľkosť" Facet Section Button
            const veľkosťFacetButton = await page.locator('button[data-testselector="size-facet"]');
            await page.waitForSelector('button[data-testselector="size-facet"]', { state: 'visible' });
            const veľkosťFacetButtonText = await veľkosťFacetButton.first().textContent();
            expect(veľkosťFacetButtonText).toContain('Veľkosť');

            const element104 = await page.locator('span.size-facet-104-notSelected');
            await expect(element104.first()).toHaveText('104');

            const element110 = await page.locator('span.size-facet-110-notSelected');
            await expect(element110.first()).toHaveText('110');

            const element110Slash116 = await page.locator('span.size-facet-110\\/116-notSelected');
            await expect(element110Slash116.first()).toHaveText('110/116');

            const element122 = await page.locator('span.size-facet-122-notSelected');
            await expect(element122.first()).toHaveText('122');

            const element122Slash128 = await page.locator('span.size-facet-122\\/128-notSelected');
            await expect(element122Slash128.first()).toHaveText('122/128');

            const element128 = await page.locator('span.size-facet-128-notSelected');
            await expect(element128.first()).toHaveText('128');

            const element134 = await page.locator('span.size-facet-134-notSelected');
            await expect(element134.first()).toHaveText('134');

            const element134Slash140 = await page.locator('span.size-facet-134\\/140-notSelected');
            await expect(element134Slash140.first()).toHaveText('134/140');

            const element140 = await page.locator('span.size-facet-140-notSelected');
            await expect(element140.first()).toHaveText('140');

            const veľkosťZobraziťVšetkoButton = await page.locator('button[data-testselector="size-facet-show-more-button"]');
            await expect(veľkosťZobraziťVšetkoButton.first()).toHaveText('Zobraziť všetko');

          } catch (error) {
            console.log("Failed to verify 'Veľkosť' facet or option: ", error);
            throw error; // re-throw the error
          }

          try {
            // Verify the "Materiál" Facet Section Button
            const materiálFacetButton  = await page.locator('button[data-testselector="Materiál-facet"]');
            await page.waitForSelector('button[data-testselector="Materiál-facet"]', { state: 'visible' });
            const materiálFacetButtonText  = await materiálFacetButton .first().textContent();
            expect(materiálFacetButtonText ).toContain('Materiál');

            const elementBavlna = await page.locator('span.Materiál-facet-Bavlna-notSelected');
            await expect(elementBavlna.first()).toHaveText('Bavlna');

            const elementPolyester = await page.locator('span.Materiál-facet-Polyester-notSelected');
            await expect(elementPolyester.first()).toHaveText('Polyester');

            const elementElastan = await page.locator('span.Materiál-facet-Elastan-notSelected');
            await expect(elementElastan.first()).toHaveText('Elastan');

            const elementLYCRA = await page.locator('span.Materiál-facet-LYCRA®-notSelected');
            await expect(elementLYCRA.first()).toHaveText('LYCRA®');

            const elementTextil = await page.locator('span.Materiál-facet-Textil-notSelected');
            await expect(elementTextil.first()).toHaveText('Textil');

            const materiálZobraziťVšetkoButton = await page.locator('button[data-testselector="Materiál-facet-show-more-button"]');
            await expect(materiálZobraziťVšetkoButton.first()).toHaveText('Zobraziť všetko');

        } catch (error) {
            console.log("Failed to verify 'Materiál' facet or option: ", error);
            throw error; // re-throw the error
        }

          try {
            // Verify the "Hodnotenia" Facet Section Button
            const hodnoteniaFacetButton  = await page.locator('button[data-testselector="Hodnotenia-facet"]');
            await page.waitForSelector('button[data-testselector="Hodnotenia-facet"]', { state: 'visible' });
            const hodnoteniaFacetButtonText = await hodnoteniaFacetButton.first().textContent();
            expect(hodnoteniaFacetButtonText).toContain('Hodnotenia');

            const element1aviac = await page.locator('span.Hodnotenia-facet-1.a.viac-notSelected');
            await expect(element1aviac.first()).toHaveText('1 a viac');

            const element2aviac = await page.locator('span.Hodnotenia-facet-2.a.viac-notSelected');
            await expect(element2aviac.first()).toHaveText('2 a viac');

            const element3aviac = await page.locator('span.Hodnotenia-facet-3.a.viac-notSelected');
            await expect(element3aviac.first()).toHaveText('3 a viac');

            const element4aviac = await page.locator('span.Hodnotenia-facet-4.a.viac-notSelected');
            await expect(element4aviac.first()).toHaveText('4 a viac');

        } catch (error) {
            console.log("Failed to verify 'Hodnotenia' facet or option: ", error);
            throw error; // re-throw the error
        }
